...
This file is manual policy source only. It does not configure, enable, authenticate, or activate any Claude connector.

## Default State

No connector is selected. No endpoint, account identifier, credential, token, private URL, authentication step, or connection payload belongs in this repository package.

## Use Conditions

A human must select, authorize, scope, and review every connector for a named task. Use the least data scope, require confirmation before write or external actions, prohibit credential copying into prompts or files, and record any data that leaves the workspace in the conversation evidence.
