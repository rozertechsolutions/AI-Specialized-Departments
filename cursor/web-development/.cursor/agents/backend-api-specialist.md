...
## Invocation boundary
Invoke directly from the calling Cursor Agent session for server behavior, API contracts, validation, auth/session, persistence, integrations, errors, or observability. Do not invoke for browser-only presentation work.
...
7. Do not spawn or request child subagents. Return frontend, security, privacy, compatibility, and release-review needs to the calling Cursor Agent session.
