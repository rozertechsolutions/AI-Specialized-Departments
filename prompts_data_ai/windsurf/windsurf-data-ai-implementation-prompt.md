# Windsurf / Cascade — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Windsurf / Cascade** in `windsurf/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `windsurf/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use current Windsurf/Cascade project-scoped `AGENTS.md`, Rules, Skills, Workflows, hooks, and permissions. Distinguish Cascade from Devin Local and do not assume identical feature support.

Expected implementation direction:
- Use `AGENTS.md` for location-based persistent instructions and Rules only when conditional activation is required.
- Use Skills for complex reusable workflows; do not duplicate them as Workflows unless explicit slash-command invocation has separate value.
- Use workspace `.windsurf/hooks.json` only for deterministic local safety/validation controls.
- Document Cascade versus Devin Local limitations where material.
- Omit memories, active MCP servers, app deploys, previews, analytics configuration, global/system hooks, credentials, and enterprise administration.


## Repository documentation rule

`windsurf/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
