# ChatGPT — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **ChatGPT** in `chatgpt/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `chatgpt/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use only the current ChatGPT product surfaces that are genuinely available to the user account: Projects and project instructions, project knowledge, native Skills, Custom GPT configuration, Workspace Agents, and apps/connectors where supported. This repository package is a manual-import/configuration package; ChatGPT does not consume fake repository agent, hook, permission, or MCP files.

Expected implementation direction:
- Create `chatgpt/data-ai/README.md` as the only general explanatory document.
- Add a concise project-instructions artifact ready to paste into a ChatGPT Project.
- Add native Skill packages only for focused, reusable workflows that benefit from progressive disclosure.
- Add separate Custom GPT or Workspace Agent instruction artifacts only when they represent a genuinely different native surface and do not duplicate the Project package.
- Describe connector/app setup only in the README. Do not add credentials, live connection definitions, or pretend that a repository file activates a connector.
- Do not create repository subagents, hooks, shell permissions, or MCP server files.


## Repository documentation rule

`chatgpt/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
