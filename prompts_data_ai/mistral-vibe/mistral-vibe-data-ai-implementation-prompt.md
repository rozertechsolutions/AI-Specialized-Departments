# Mistral Vibe — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Mistral Vibe** in `mistral-vibe/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `mistral-vibe/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use the current Mistral Vibe configuration shared by CLI, VS Code, and supported web/ACP surfaces. Do not use deprecated Mistral Code Enterprise formats.

Expected implementation direction:
- Use project `.vibe/` configuration, custom agents, Skills, permissions, and hooks according to current official schemas.
- Restrict tools per specialist and preserve explicit approvals.
- Hooks must be local-only, network-free, deterministic, and non-destructive.
- Do not configure providers, API keys, profiles, telemetry choices, ACP installation, or active MCP servers.
- Do not install Vibe or require a specific model.
- Document trusted-folder behavior and surface limitations in the README.


## Repository documentation rule

`mistral-vibe/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
