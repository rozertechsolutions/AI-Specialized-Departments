# Claude Code — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Claude Code** in `claude-code/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `claude-code/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use Claude Code-native, project-scoped repository configuration only. Verify the current schemas before writing files.

Expected implementation direction:
- Use `CLAUDE.md` or the current official project instruction mechanism.
- Use project-scoped custom subagents for non-overlapping specialist responsibilities where delegation provides real value.
- Use Agent Skills for focused workflows and reference material.
- Use project settings and deterministic hooks only for real safety or validation controls. Hooks must be reviewed, local-only, non-destructive, network-free, and disabled or approval-gated where the official format permits.
- Do not configure active MCP servers by default. Omit MCP entirely unless a safe disabled example is natively valid and provides clear value.
- Keep subagent nesting bounded and prevent agents from approving their own work.


## Repository documentation rule

`claude-code/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
