# GitHub Copilot — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **GitHub Copilot** in `github-copilot/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `github-copilot/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use current GitHub Copilot repository customizations and clearly distinguish IDE, CLI, GitHub.com cloud agent, and code review surfaces.

Expected implementation direction:
- Use repository custom instructions and path-specific instructions where needed.
- Use `.github/agents/*.agent.md` or the current official custom-agent location for bounded specialist agents.
- Use Agent Skills in the current supported repository location.
- Add prompt files or hooks only when they are natively supported on the selected surface and provide non-duplicated value.
- Mark preview-only capabilities and surface limitations in the README.
- Do not configure active MCP servers: cloud agents can invoke configured MCP tools autonomously, so omit them unless the user explicitly approves a later integration.
- Do not add Actions workflows, secrets, variables, firewall changes, scheduled automation, commits, pull requests, or code-review automation.


## Repository documentation rule

`github-copilot/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
