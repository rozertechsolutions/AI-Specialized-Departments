# Claude — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Claude** in `claude/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `claude/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use only Claude web/desktop product capabilities: Projects, project instructions, project knowledge, custom Skills, artifacts where relevant, and connectors/integrations. Do not mix in Claude Code repository formats.

Expected implementation direction:
- Create `claude/data-ai/README.md` as the only general explanatory document.
- Add a project-instructions artifact suitable for Claude Projects.
- Add carefully scoped native Skill packages and only the knowledge/reference assets required by those Skills.
- Put manual connector or integration setup, plan limitations, and import steps in the README only.
- Do not create `.claude/`, `CLAUDE.md`, Claude Code subagents, hooks, shell permissions, or project MCP files.


## Repository documentation rule

`claude/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
