# Warp / Oz — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Warp / Oz** in `warp/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `warp/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use Warp's current local Agent/Oz surface, project Rules (`AGENTS.md` or `WARP.md`), Agent Skills, Warp Drive workflows/notebooks only when they are genuinely importable or manually configurable, and Agent Profiles/permissions.

Expected implementation direction:
- Create project rules and Skills that Warp can discover from the repository.
- Provide any manual Warp Drive workflow or Agent Profile configuration as directly usable configuration content, with import steps only in the README.
- Do not simulate custom repository subagent manifests if Warp does not support them.
- Do not create autonomous cloud agents, event triggers, background tasks, environment variables, active MCP servers, team objects, or credentials.
- Keep command execution approval-controlled and do not define workflows that deploy, publish, mutate Git, or access external systems.


## Repository documentation rule

`warp/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
