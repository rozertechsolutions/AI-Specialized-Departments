# Qwen Code — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Qwen Code** in `qwen-code/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `qwen-code/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use current Qwen Code project context/instructions, declarative subagents, Agent Skills, hooks, custom commands, settings, and extension capabilities.

Expected implementation direction:
- Verify the current project instruction filename and all project-scoped paths before writing files.
- Use declarative subagents for specialist roles with per-agent tool, MCP, and hook boundaries where currently supported.
- Use Agent Skills and explicit commands for reusable procedures.
- Hooks must be deterministic, local-only, network-free, and non-destructive.
- Do not create an extension merely as a packaging wrapper unless it is the only native way to deliver required components.
- Omit active MCP servers, authentication, provider configuration, model selection, unattended loops, and broad shell permissions.


## Repository documentation rule

`qwen-code/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
