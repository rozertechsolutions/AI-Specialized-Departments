# JetBrains Junie — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **JetBrains Junie** in `junie/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `junie/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use Junie's current IDE/CLI project-scoped guidelines, Agent Skills, custom subagents, commands, and MCP support. Do not mix Junie with generic JetBrains AI Assistant configuration.

Expected implementation direction:
- Use `.junie/AGENTS.md` or the currently documented project guideline path, avoiding legacy `guidelines.md` unless required for compatibility.
- Use project-scoped custom subagents with explicit read/write and tool restrictions.
- Use Agent Skills and shared commands only for focused workflows.
- Keep MCP disabled and unconfigured by default.
- Do not create a GitHub Action, extension package, global cache content, model settings, or environment-variable configuration.
- Document IDE versus CLI availability and any preview limitations in the README.


## Repository documentation rule

`junie/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
