# Codex — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Codex** in `codex/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `codex/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use current Codex project-scoped mechanisms only.

Expected implementation direction:
- Use `AGENTS.md` for durable specialization instructions.
- Use `.codex/agents/*.toml` for genuinely distinct custom agents, with explicit tool/sandbox limits and no recursive delegation beyond what is required.
- Use `.agents/skills/<skill>/SKILL.md` for focused reusable workflows.
- Use `.codex/config.toml`, project rules, and project hooks only where needed for least privilege and deterministic validation.
- Project hooks must be local-only, network-free, non-destructive, portable, and documented in the README because project trust is required.
- Do not add active MCP servers, provider configuration, credentials, global settings, worktree setup, scheduled tasks, plugins, or Git automation.
- Do not hardcode a model. Respect the user's selected model and reasoning configuration.


## Repository documentation rule

`codex/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
