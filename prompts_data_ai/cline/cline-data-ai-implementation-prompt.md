# Cline — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Cline** in `cline/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `cline/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use Cline's current project-scoped `.cline/` configuration model and only features supported by the chosen Cline IDE/CLI surface.

Expected implementation direction:
- Use project rules for durable Data and AI constraints.
- Use project agent definitions only where the current official schema supports them; do not mistake experimental read-only research subagents for fully configurable implementation agents.
- Use Skills for focused reusable procedures.
- Use lifecycle hooks only for deterministic, local, non-destructive checks with explicit safe failure behavior.
- Keep approval requirements enabled. Do not enable broad auto-approval.
- Do not add active MCP servers, plugins, schedules, cron jobs, Kanban automation, or provider credentials.
- Account for differences between IDE, CLI, SDK, and experimental features in the README.


## Repository documentation rule

`cline/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
