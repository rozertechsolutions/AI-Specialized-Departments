# Cursor — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Cursor** in `cursor/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `cursor/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use the current Cursor rules, subagents, Agent Skills, hooks, permissions, and MCP surfaces only after confirming their release channel and project-scoped file formats.

Expected implementation direction:
- Use project Rules or `AGENTS.md` according to the current recommended format; do not retain legacy formats unless official documentation still requires them.
- Use custom subagents only for bounded specialist work with clear return contracts.
- Use Skills for repeatable Data and AI procedures.
- Use workspace hooks only for deterministic local validation and policy enforcement, never for background publication or external actions.
- Do not require Nightly/preview-only features without documenting that limitation and providing a stable fallback based on rules/instructions.
- Omit active MCP configuration and credentials.
- Do not create cloud-agent, worktree, or marketplace/plugin setup unless it is essential and safely disabled.


## Repository documentation rule

`cursor/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
