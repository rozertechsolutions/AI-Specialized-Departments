# OpenAI Agents SDK — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **OpenAI Agents SDK** in `openai-agents-sdk/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `openai-agents-sdk/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Implement a real, minimal Python package using the current stable OpenAI Agents SDK. This is the only platform in this set that should contain executable orchestration code by default.

Expected implementation direction:
- Build real `Agent` objects for the minimum complete, non-overlapping specialist model and one coordinator.
- Use explicit handoffs or agents-as-tools according to ownership boundaries; prevent circular delegation and uncontrolled recursion.
- Implement structured input/output contracts, tool wrappers, input/output/tool guardrails, human approval for sensitive actions, run context, and serialization helpers where justified.
- Include offline construction, routing, guardrail, approval, and serialization tests. Tests must not call a model, network, hosted tracing, or external service.
- No API keys, live endpoints, server, web UI, deployment, installer, database, background worker, or cloud infrastructure.
- Disable or replace hosted tracing in tests and ensure no sensitive data is emitted.
- Create a temporary isolated virtual environment only for final SDK compatibility verification. Install dependencies only inside it, capture exact package versions on screen, run the tests, and remove the environment completely before finishing. Do not commit the environment, caches, coverage files, or test reports.


## Repository documentation rule

`openai-agents-sdk/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
