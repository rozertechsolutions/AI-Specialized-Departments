# Kiro — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Kiro** in `kiro/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `kiro/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use Kiro's current workspace steering, custom agents, Agent Skills, hooks, specs, and permissions. Choose IDE or CLI formats based on the repository's existing Kiro implementation and current official documentation.

Expected implementation direction:
- Use steering files or `AGENTS.md` for persistent specialization rules.
- Use custom agents and Skills for bounded responsibilities and reusable procedures.
- Use Specs only when they are the native executable workflow representation for complex Data and AI work; do not generate sample specs or explanatory requirements/design/tasks merely to demonstrate the feature.
- Hooks must be local-only, deterministic, non-destructive, and disabled unless a safe default action is justified.
- Do not add Powers or active MCP servers, AWS-specific configuration, credentials, shell automation, or deployment tasks.


## Repository documentation rule

`kiro/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
