# OpenCode — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **OpenCode** in `opencode/data-ai/`, using only current native capabilities and the repository's established conventions.
...
2. Work only inside `opencode/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use current OpenCode project-scoped rules, agents/subagents, Skills, commands, tools, permissions, plugins, and MCP support.

Expected implementation direction:
- Use `AGENTS.md` for durable instructions.
- Use `.opencode/` project agent definitions for bounded specialists and commands for explicit workflows.
- Use Agent Skills for reusable Data and AI procedures.
- Configure conservative permissions; shell and external actions must ask or be denied, and destructive/Git commands must be denied.
- Avoid plugins unless a deterministic hook cannot be expressed natively and the plugin provides essential value. Any plugin must be small, local-only, dependency-free where practical, and tested.
- Omit active MCP servers, custom providers, credentials, share settings, and global configuration.


## Repository documentation rule

`opencode/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
