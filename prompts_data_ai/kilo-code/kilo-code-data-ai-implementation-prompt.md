# Kilo Code — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Kilo Code** in `kilo-code/data-ai/`, using only current native capabilities and the repository's established conventions.
...
- https://kilocode.ai/docs/features/mcp/overview
...
2. Work only inside `kilo-code/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
Use Kilo Code's current project rules, custom agents/modes, Agent Skills, workflows/slash commands, subagent delegation, and permission model.

Expected implementation direction:
- Prefer current Kilo paths and formats; do not use deprecated Memory Bank or deprecated Orchestrator Mode.
- Use custom agents for distinct specialist roles with explicit file/tool permissions.
- Use Skills for reusable expertise and project workflows/slash commands for explicit user-invoked procedures.
- Keep auto-approval conservative and never enable broad unattended execution.
- Omit active MCP servers, hosted KiloClaw infrastructure, Gateway configuration, organization management, and provider credentials.
- Document differences between VS Code, JetBrains, and CLI surfaces where relevant.


## Repository documentation rule

`kilo-code/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
