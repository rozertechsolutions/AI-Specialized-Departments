# Local provider-agnostic platform — Data and AI specialization implementation prompt

## Objective

Implement the complete, professional **Data and AI** specialization for **Local provider-agnostic platform** in `local/data-ai/`, using only current native capabilities and the repository's established conventions.
...
- No single vendor documentation applies. Validate all chosen open formats against their official specifications and the repository requirements.
...
2. Work only inside `local/data-ai`. Preserve every other platform, specialization, untracked file, and user change.
...
`local/` is an independent open, provider-agnostic specification/configuration package, not an Ollama or LM Studio implementation and not a universal cross-platform runtime.

Expected implementation direction:
- Define a coherent, documented local package under `local/data-ai/` with one `README.md`, provider-neutral schemas/configuration, specialist definitions, skills, workflows, safety policies, and validation assets.
- All providers, runtimes, models, endpoints, tools, storage backends, vector stores, execution capabilities, and permissions must be user-selectable and disabled by default.
- Optional examples for Ollama, LM Studio, OpenAI-compatible endpoints, or other runtimes may exist only as clearly disabled, non-secret examples and must not make any provider the default.
- Do not create a daemon, server, UI, installer, universal adapter, compatibility layer, orchestration framework, model downloader, dependency installer, or executable runtime.
- Use portable open formats and JSON Schema or equivalent validation where useful.
- Any example path or endpoint must be non-sensitive, local-only, and obviously replaceable.


## Repository documentation rule

`local/data-ai/README.md` must be the single general explanatory document for this platform implementation. It must contain:
