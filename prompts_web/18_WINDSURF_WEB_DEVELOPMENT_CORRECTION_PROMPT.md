...
`TARGET_DIRECTORY = windsurf/web-development/`

**Target product surface:** Current Devin Desktop Cascade rules, AGENTS.md, Skills, Workflows, hooks, and MCP, with explicit distinction from Devin Local Agent capabilities.
...
**Official documentation baseline to verify:** Windsurf/Devin Desktop Docs: Skills; Windsurf/Devin Desktop Docs: Memories and Rules; Windsurf/Devin Desktop Docs: AGENTS.md; Windsurf/Devin Desktop Docs: Devin Local Agent; Windsurf/Devin Desktop Docs: Hooks and MCP only to document omission.

## 4. Known audit findings to correct

- The package uses .windsurf/rules, .windsurf/skills, .windsurf/workflows, and AGENTS.md but does not clearly state whether it targets Cascade or Devin Local.
- Workflows are available in Cascade but not in Devin Local, while both can use rules/AGENTS.md and Skills.
- The content is generic and may load duplicate guidance through AGENTS.md and rules.
- Hooks and MCP are supported surfaces but are inappropriate for this static safe package.
## 5. Required platform-native corrections

1. Target current Cascade in Devin Desktop as the primary surface for rules, Skills, and Workflows. Document Devin Local compatibility separately.
1. Validate .windsurf/rules paths, frontmatter, and activation modes. Use rules for behavioral guidance and quality gates, not full workflow procedures.
1. Validate .windsurf/skills and Skill metadata. Keep reusable multi-step procedures with precise triggers, inputs, outputs, evidence, and stop conditions.
1. Validate .windsurf/workflows as Cascade-only manual slash-command templates. Do not claim they are available to Devin Local.
1. Use AGENTS.md only for concise location-based guidance shared by supported surfaces. Remove duplicate full bodies from AGENTS.md and rules.
1. Do not add Cascade hooks, hook scripts, MCP configuration, Enterprise system rules, deployments, browser automation, App Deploys, or external integrations.
1. Document exact differences among Cascade, Devin Local, and any legacy Windsurf behavior in README.md.
1. Do not create custom subagent files unless current official documentation exposes a stable repository mechanism. Fast Context internal subagents are not user-configurable department agents.
1. Ensure every workflow ends with evidence-based PASS, FAIL, or BLOCKED and requires human review for sensitive changes.
...
- .windsurf/workflows/*.md for Cascade
- AGENTS.md where non-duplicative
### Omit or delete
...
- Enterprise rules
- Simulated custom agents
- Workflows presented as Devin Local compatible
...
- Each component is mapped to Cascade and/or Devin Local accurately.
- Workflows are explicitly Cascade-only.
- AGENTS.md and rules do not duplicate complete instructions.
- No hooks or MCP files exist.
- README uses current Devin Desktop terminology without hiding legacy path compatibility.
