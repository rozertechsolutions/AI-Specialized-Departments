...
`TARGET_DIRECTORY = kiro/web-development/`

**Target product surface:** Kiro IDE project custom agents, steering, Agent Skills, permissions, hooks, powers, and MCP only where currently documented and justified.
...
**Official documentation baseline to verify:** Kiro Docs: Custom agents; Kiro Docs: Agent configuration reference; Kiro Docs: Agent Skills; Kiro Docs: Steering, hooks, and MCP configuration.

## 4. Known audit findings to correct

- Custom agents list unsupported tool tags grep and glob; current documented categories include read, write, shell, web, subagent, context, @mcp, @builtin, and wildcard.
- The lead lacks the subagent category and therefore cannot delegate as described.
- The standalone .kiro/settings/mcp.json is empty.
- Agent Skills and steering need an explicit responsibility split and correct resource inheritance behavior.
- Hooks are executable and must not be added.
## 5. Required platform-native corrections

1. Delete .kiro/settings/mcp.json because it contains no server. Do not add MCP or powers.
1. Replace unsupported grep and glob tags with the current read category, which includes file reading, listing, and searching.
1. Give the lead the exact categories required for orchestration: read, write only when it owns integration edits, subagent for delegation, and web only if documentation research is explicitly allowed. Do not grant shell, @mcp, @builtin, or wildcard.
1. Give implementation specialists read and narrowly justified write. Give reviewers read only. Add explicit filesystem denials for secrets, environment files, credentials, and paths outside the specialization where supported.
1. Validate every agent's YAML frontmatter and permission rules against the current Kiro IDE custom-agent schema.
1. Ensure custom agents inherit or explicitly reference the intended steering and Skills according to current Kiro behavior. Do not assume CLI resource fields apply to IDE markdown agents.
1. Use steering for always-on Web Development rules and quality gates; use Skills for on-demand procedures. Remove duplicate full instruction bodies.
1. Do not create hooks, command actions, MCP, or external integrations.
1. Define explicit lead-to-specialist delegation, bounded results, independent reviews, and final conflict reconciliation.
...
- grep and glob tool tags
- Empty MCP settings
- Hooks
- Powers
- Shell
- Wildcard or broad built-in access
...
- Every tool tag is a documented Kiro category.
- The lead includes subagent access and reviewers do not.
- No MCP settings file remains.
- Steering and Skills have non-overlapping activation roles.
- Permission rules default safely and protect sensitive paths.
