...
`TARGET_DIRECTORY = cursor/web-development/`

**Target product surface:** Cursor Project Rules, AGENTS.md, project subagents, Agent Skills, hooks, MCP, Plan mode, and CLI/cloud surfaces only where current official documentation supports them.
...
**Official documentation baseline to verify:** Cursor Docs: Rules; Cursor Docs: Subagents; Cursor Docs: Agent Skills; Cursor Docs: Hooks; Cursor Docs: MCP and CLI.

## 4. Known audit findings to correct

- The empty .cursor/mcp.json is a placeholder and provides no capability.
- AGENTS.md and .cursor/rules can load overlapping instructions and need a single authoritative responsibility split.
- The package uses native-looking agents and Skills but lacks strong platform-specific invocation, permission, and review contracts.
- Hooks would be executable and must not be added under this static policy.
## 5. Required platform-native corrections

1. Delete the empty .cursor/mcp.json. Do not replace it unless a concrete optional integration is indispensable; this package requires none.
1. Choose a non-duplicative instruction strategy. Use Project Rules for Cursor-specific activation and quality gates; retain AGENTS.md only for concise location-based guidance that does not repeat the rules, or delete it if redundant.
1. Validate all .cursor/rules/*.mdc frontmatter, activation modes, glob scopes, and always-apply behavior against current Cursor documentation.
1. Validate .cursor/agents/*.md against the current subagent format. Define a lead only if Cursor supports the intended coordinator behavior; otherwise make specialists independently invocable and remove false delegation claims.
1. Grant implementation roles only necessary read/write capabilities. Keep security, accessibility/performance/SEO, and quality reviewers read-only.
1. Validate .cursor/skills discovery and Skill metadata. Separate evergreen domain Skills from workflow Skills and remove overlapping procedures.
1. Do not add hooks, scripts, cloud-agent automation, GitHub integrations, or MCP. Mention those surfaces only as unsupported or user-controlled limitations in README.md.
1. Document whether each component applies to the desktop IDE, Cursor CLI, or Cloud Agents. Do not imply universal support across surfaces.
1. Add exact final evidence requirements and a reconciliation step for independent reviewer findings.
...
- Empty MCP
- Executable hooks
- Automatic Cloud Agent runs
- Unverified cross-surface behavior
...
- Every agent, rule, and Skill path matches current Cursor documentation.
- Only one source owns each always-on instruction.
- The lead's coordination claim matches actual Cursor subagent capabilities.
- README maps each component to IDE, CLI, or cloud surface and states limitations.
