...
`TARGET_DIRECTORY = junie/web-development/`

**Target product surface:** Current Junie IDE and Junie CLI project guidelines, AGENTS.md, Agent Skills, custom agents, commands, MCP, and other documented configuration surfaces.
...
**Official documentation baseline to verify:** Junie Documentation: Guidelines and memory; Junie Documentation: Current customization pages; JetBrains Junie official blog only as secondary product-surface evidence.

## 4. Known audit findings to correct

- The existing implementation is honest but minimal, containing only AGENTS.md and README.md.
- Current Junie CLI documentation gives .junie/AGENTS.md precedence over root AGENTS.md and now exposes more customization capabilities than the existing package uses.
- The package needs a deliberate decision between a minimal cross-surface guideline package and richer CLI-native components.
- No unsupported mechanism should be added merely because recent product announcements mention it.
## 5. Required platform-native corrections

1. Verify the exact current Junie IDE and Junie CLI surfaces before changing structure.
1. Use .junie/AGENTS.md as the project-specific guideline source if current official documentation recommends it. Remove or reduce root AGENTS.md to avoid duplicate loading; never keep identical copies.
1. Rewrite guidelines as concise, evidence-based Web Development instructions with stack preservation, safety, review, and completion behavior.
1. Add project Agent Skills only if the current official Junie documentation specifies a stable repository path and schema. Skills must be instruction-only and focused.
1. Add custom agents or commands only when the official documentation—not only a product blog—defines their project-scoped format, discovery path, and supported permissions.
1. Do not add MCP, hooks, provider keys, model configuration, installations, or authentication.
1. Document differences between Junie IDE and Junie CLI, including which components are discovered by each surface.
1. Preserve a minimal implementation when richer mechanisms remain preview, undocumented, user-level only, or not version-controlled.
1. Update README.md with the chosen product surface, exact discovery behavior, omissions, and static verification status.
## 6. Native capability boundary

### Retain or create only when verified native

- .junie/AGENTS.md or root AGENTS.md according to current precedence
- Project Skills or custom agents only when officially documented
### Omit or delete

- Legacy duplicate guidelines
- Undocumented preview files
- MCP
- Hooks
...
- Only one project guideline source contains the full instructions.
- Every additional component has an official documented project path.
- IDE and CLI support are not conflated.
- Minimality is accepted when it is the most honest native result.
