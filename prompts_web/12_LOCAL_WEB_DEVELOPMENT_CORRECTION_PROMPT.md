...
`TARGET_DIRECTORY = local/web-development/`

**Target product surface:** A provider-, runtime-, and model-neutral static specification package for local AI implementations. It is not an executable local runtime and must not be bound to Ollama, LM Studio, or any specific model.
...
**Official documentation baseline to verify:** The project's provider-neutral local architecture requirements; Agent Skills open specification where Skills are retained; JSON Schema official specification for schema correctness.

## 4. Known audit findings to correct

- The current package is schema-valid and appropriately provider-neutral, but role, workflow, evidence, and lifecycle contracts remain too brief.
- The package must make the distinction between specification and executable runtime impossible to misunderstand.
- Permissions and integrations are disabled by default but require stronger schema-level invariants and documentation.
- Cross-file references and duplicate conceptual content require consolidation.
## 5. Required platform-native corrections

1. Preserve the local package as a declarative specification only. Do not add Python, JavaScript, shell, Docker, installers, adapters, servers, or runtime code.
1. Audit department.yaml and department.schema.json together. Add schema constraints that require explicit provider/runtime/model selection by the downstream user and safe disabled defaults.
1. Ensure integrations.json contains no active integration, endpoint, command, token, provider, or auto-approval. Empty or disabled examples must be meaningful and schema-valid rather than placeholders.
1. Strengthen permissions.json with least-privilege categories, explicit denied operations, human-review actions, and a clear non-enforcement disclaimer for runtimes that consume the specification.
1. Expand roles/*.md and workflows/*.md to include the complete operational contract fields, deterministic handoff artifacts, stop conditions, failure states, and evidence requirements.
1. Ensure reviewer roles are read-only by specification and cannot close their own findings. Define lead reconciliation without self-approval.
1. Make provider examples generic and disabled. Ollama and LM Studio may be named only as non-binding examples, never defaults.
1. Remove duplicate prose by assigning one source of truth for department scope, quality gates, permissions, roles, and workflows.
1. Update README.md with implementation guidance for downstream runtimes, conformance expectations, extension points, and explicit non-runtime status.
...
- department.yaml
- JSON Schema
- Provider-neutral policy JSON
- Markdown role and workflow contracts
- Agent Skills as neutral instruction packages
### Omit or delete
...
- department.yaml validates conceptually against the updated schema.
- All defaults are disabled or deny/ask; nothing grants execution automatically.
- Every role and workflow identifier resolves consistently across files.
- No provider, model, endpoint, or local path is mandatory.
- README clearly states that conformance must be implemented by a separate user-selected runtime.
