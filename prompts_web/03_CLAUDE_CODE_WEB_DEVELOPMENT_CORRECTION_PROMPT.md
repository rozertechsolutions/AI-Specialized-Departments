...
`TARGET_DIRECTORY = claude-code/web-development/`

**Target product surface:** Claude Code project CLAUDE.md, project agents, Agent Skills, project settings, permissions, hooks, and MCP only where current official documentation supports them.
...
**Official documentation baseline to verify:** Claude Code Docs: Create custom subagents; Claude Code Docs: Extend Claude with skills; Claude Code Docs: Tools reference; Claude Code Docs: Hooks and settings.

## 4. Known audit findings to correct

- The web-development-lead agent omits the Agent tool and therefore cannot delegate to the specialist agents it claims to coordinate.
- Agent tool allowlists omit Skill access or skill preloading, so the supplied Skills are not integrated into the agent workflow.
- The standalone .mcp.json contains no servers and is an empty placeholder.
- The project settings file disables all hooks even though the package contains no hooks, which can unnecessarily override user or project behavior.
- Role contracts are too brief and do not fully define delegation, evidence, failure, and reconciliation behavior.
## 5. Required platform-native corrections

1. Delete the empty .mcp.json. Do not replace it unless a concrete, optional, disabled-by-default MCP server has indispensable value; no such integration is required for this package.
1. Audit .claude/settings.json. Remove it if its only effect is disableAllHooks. Do not globally suppress user hooks merely because this package contains none.
1. Configure web-development-lead as a valid main-session custom agent that can be launched explicitly and can invoke only the approved specialist agents through the native Agent tool.
1. Use the current Agent(agent-name, ...) allowlist syntax where supported for the main-session lead. Prevent arbitrary or recursive delegation.
1. Integrate Skills correctly. Use the native skills frontmatter field for deliberate preloading and/or include the Skill tool where on-demand activation is required. Do not list nonexistent tools.
1. Give architecture, frontend, and backend implementers only the file tools needed for their role. Do not grant Bash. Give security, accessibility/performance/SEO, and quality reviewers read-only tools and prohibit edits.
1. Define exact delegation order: discovery and architecture before implementation; security and accessibility reviews after relevant changes; final quality review last. The lead must reconcile findings but cannot close reviewer findings without evidence or human acceptance.
1. Keep hooks omitted because this package must not add executable scripts. Do not simulate safety hooks with documentation claiming enforcement.
1. Rewrite CLAUDE.md as concise project-level orchestration and safety guidance rather than duplicating complete agent instructions.
1. Validate every agent frontmatter field, tool name, Skill name, and project path against the current Claude Code version.
...
- Project settings when materially needed
- Native Agent delegation and Skill loading
### Omit or delete

- Empty .mcp.json
- Executable hooks
- Bash access
- Unbounded nested subagents
- Fake workflow files
...
- The lead can actually invoke the exact approved specialists.
- Every supplied Skill is either deliberately preloaded or available through the native Skill mechanism.
- Reviewers cannot edit files or invoke implementation agents.
- No settings file changes unrelated user behavior.
- All agent names referenced in instructions and allowlists exactly match their frontmatter names.
