...
`TARGET_DIRECTORY = cline/web-development/`

**Target product surface:** Cline project Rules and Agent Skills. Hooks, plugins, MCP, and other mechanisms only if current official documentation confirms project-scoped value and the package can remain static and safe.
...
**Official documentation baseline to verify:** Cline Docs: Rules; Cline Docs: Skills; Cline Docs: Configuration security notes; Cline Docs: Plan and Act mode.

## 4. Known audit findings to correct

- The current .clinerules and .cline/skills paths are plausible native mechanisms, but the content is generic and lacks full operational contracts.
- The package does not justify custom agents or automated orchestration, and it must not simulate them.
- Hooks and plugins can execute code and are therefore inappropriate for this static safe-by-default package.
- Rules and Skills need clearer activation boundaries to avoid always-loading task procedures.
## 5. Required platform-native corrections

1. Retain .clinerules as the authoritative always-on behavioral layer only if current official documentation still identifies it as the primary project rule format.
1. Keep always-on rules short: scope, stack preservation, safety, evidence, human review, and completion truthfulness. Move task procedures out of rules and into Skills.
1. Validate .cline/skills discovery and SKILL.md metadata. Retain only focused on-demand Skills with clear triggering and non-triggering conditions.
1. Do not create custom agent files unless current official Cline documentation exposes a real project-scoped custom-agent mechanism. Rules and Skills must not pretend to be agents.
1. Do not add hooks, plugins, scripts, or MCP configuration. Their executable or external nature conflicts with this package's static constraints.
1. Rewrite the quality and security rules to define explicit human approval gates without claiming technical enforcement.
1. Expand each Skill with exact inputs, outputs, evidence, stop conditions, failure behavior, and review requirements.
1. Update README.md with Rule and Skill locations, activation behavior, manual use, unsupported mechanisms, and static verification limits.
...
- Cline task prompts entered by the user
### Omit or delete
...
- Scripts
- Empty MCP
- Automatic terminal or deployment actions
...
- Always-on rules contain no long workflow procedures.
- Skills are discoverable, focused, and non-overlapping.
- The README does not promise multi-agent orchestration.
- Safety statements are presented as model instructions and human gates, not hard enforcement.
