...
`TARGET_DIRECTORY = gemini-cli/web-development/`

**Target product surface:** Gemini CLI GEMINI.md context, project custom commands, project subagents, Agent Skills, settings, hooks, extensions, and MCP only where current official documentation supports them.
...
**Official documentation baseline to verify:** Gemini CLI official repository: Custom commands; Gemini CLI official repository: Subagents; Gemini CLI official repository: Agent Skills; Gemini CLI official repository: Settings and hooks.

## 4. Known audit findings to correct

- The current .gemini/settings.json is empty and must be removed.
- The package contains commands and GEMINI.md but omits currently documented project subagents and Agent Skills, leaving professional coverage incomplete.
- Hooks are executable and project policy forbids adding them.
- Product features have evolved quickly, so every path and field must be checked against the current official repository documentation.
## 5. Required platform-native corrections

1. Delete the empty .gemini/settings.json. Create a settings file only if a current, non-empty, security-relevant project setting is required; do not add empty MCP, hook, or experimental flags.
1. Retain and refine GEMINI.md as concise root coordination, safety, stack-preservation, and evidence guidance.
1. Validate all .gemini/commands/**/*.toml files against the current custom command schema. Ensure descriptions, prompts, argument handling, and command names are correct and non-overlapping.
1. Add project subagents under the current official .gemini/agents location if the installed current product surface supports them. Use exact supported frontmatter only; do not use planned or unimplemented fields.
1. Design the root Gemini session as coordinator unless official nested delegation is stable and necessary. Reviewers must be read-only; implementation specialists may edit but must not receive shell tools.
1. Add project Skills under the officially documented .gemini/skills location. Reuse the professional coverage model, but rewrite Skills for Gemini's activation and tool model rather than copying another platform.
1. Do not add hooks, hook scripts, extensions, remote agents, A2A, or MCP. These are not required for this static specialization and would add execution or external trust.
1. Document feature maturity, current CLI version assumptions, and any preview limitations in README.md.
1. Ensure custom commands invoke or instruct the correct coordinator/specialist flow without claiming that a command itself enforces execution or approvals.
...
- .gemini/agents/*.md where currently supported
- .gemini/skills/*/SKILL.md where currently supported
### Omit or delete
...
- Remote agents
- MCP
- Experimental policy files with nonfunctional workspace enforcement
...
- Every command parses according to the current TOML schema and namespaced command discovery rules.
- Every agent uses only currently implemented frontmatter fields and valid built-in tool names.
- Skills use the current workspace discovery path and valid metadata.
- No preview feature is described as stable without official confirmation.
- The README clearly identifies static-only verification.
