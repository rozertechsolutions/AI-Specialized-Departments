...
`TARGET_DIRECTORY = qwen-code/web-development/`

**Target product surface:** Qwen Code context files, project subagents, Agent Skills, current project settings, hooks, extensions, MCP, and agent teams only where stable, documented, and justified.
...
**Official documentation baseline to verify:** Qwen Code Docs: Subagents; Qwen Code Docs: Configuration settings; Qwen Code Docs: Agent Skills; Qwen Code Docs: Hooks limitations; Current official weekly update for nested subagents only as version evidence.

## 4. Known audit findings to correct

- model.maxSubagentDepth is set to 1, which explicitly prevents a lead subagent from spawning specialist children.
- The lead's tool list does not include the current native agent invocation tool.
- The settings file contains an empty mcpServers object.
- Some possible declarative-agent fields remain planned or partially implemented and must not be used without current documentation.
- Per-agent hooks have global firing limitations and executable hooks are prohibited.
## 5. Required platform-native corrections

1. Remove the empty mcpServers object. Do not configure MCP.
1. Choose one safe orchestration topology after checking current official documentation: either root QWEN.md coordination with direct specialists at depth 1, or a lead subagent with maxSubagentDepth set to exactly 2. Prefer the smallest depth that supports the design.
1. If retaining a lead subagent, add the exact current native agent invocation tool and restrict delegation in instructions to the six approved specialists. Add explicit anti-recursion and maximum-one-specialist-layer rules.
1. Use only currently implemented subagent frontmatter fields. Omit effort, memory, initialPrompt, skills, isolation, or other fields when official documentation says they land in future work.
1. Give implementation specialists only current read/write file tools; deny shell. Give reviewers read-only tools and no agent-spawning capability.
1. Validate .qwen/settings.json against the current organized configuration format and place maxSubagentDepth in the correct category.
1. Validate .qwen/agents and .qwen/skills paths, metadata, and names against current Qwen Code docs.
1. Do not add hooks, extensions, agent teams, remote agents, MCP, channels, loops, scheduling, worktrees, or auto-approval. These are outside this static package.
1. Rewrite QWEN.md as concise coordinator and safety context, not a duplicate of all role prompts.
1. Document current version assumptions and nested-subagent behavior in README.md.
...
- .qwen/settings.json with bounded depth
### Omit or delete

- maxSubagentDepth 1 with nested lead
- Empty MCP
- Hooks
- Extensions
- Agent teams
- Auto-approval
- Unimplemented frontmatter fields
...
- The selected depth permits exactly the intended hierarchy and no more.
- The lead has the real agent invocation tool if it must delegate.
- Reviewers cannot write or spawn agents.
- Every frontmatter field is documented as implemented, not planned.
- Settings use the current organized schema.
