...
`TARGET_DIRECTORY = claude/web-development/`

**Target product surface:** Claude web/desktop Projects, project instructions, project knowledge, custom Skills, and connectors. This is not Claude Code.
...
**Official documentation baseline to verify:** Anthropic Help Center: Claude Projects; Anthropic Help Center: What are Skills?; Current official Claude connector and workspace documentation.

## 4. Known audit findings to correct

- The current package is a manual Claude Projects package and cannot act as a repository coding agent.
- Project instructions, project knowledge, setup guidance, and Skills require stronger separation of purpose and plan limitations.
- The current Skills and instruction bodies are heavily generic and insufficiently adapted to Claude's manual Project workflow.
- Connectors must not be configured, authenticated, or trusted by default.
## 5. Required platform-native corrections

1. Keep the implementation strictly within Claude web/desktop product capabilities. Remove every Claude Code concept, filename, permission model, hook, subagent, or repository automation claim.
1. Rewrite project-instructions.md for manual insertion into a Claude Project. It must coordinate professional Web Development reasoning without claiming tools or automatic file access.
1. Refine project-knowledge/*.md into non-behavioral reference documents. Eliminate overlap with project instructions and between knowledge files.
1. Validate every skill against current Claude custom Skills requirements. Retain only distinct, reusable workflows and instruction packages; do not include executable resources.
1. Rewrite claude-project-setup.md as an exact manual setup procedure covering project creation, instruction insertion, knowledge upload, optional Skill addition, and verification.
1. Document plan, account, workspace, connector, and administration limitations precisely.
1. Mention connectors only as optional user-controlled integrations. Do not include active connector configuration, tokens, URLs, OAuth instructions, or automatic external actions.
1. Update README.md with capability classification, manual installation mapping, known limitations, and evidence-based completion criteria.
...
- Project knowledge
- Custom Skills where available
- Optional manually configured connectors
### Omit or delete

- Claude Code files
- Repository agents or subagents
- Hooks
- Shell permissions
- Automatic MCP connections
...
- No Claude Code terminology or file mechanism remains.
- Every file is usable through a documented Claude web/desktop UI flow.
- Knowledge is reference content; instructions are behavior; Skills are on-demand procedures.
- Availability and manual limitations are explicit.
