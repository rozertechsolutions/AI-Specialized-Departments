...
`TARGET_DIRECTORY = mistral-vibe/web-development/`

**Target product surface:** Current Mistral Vibe project AGENTS.md, project Skills, custom agents, prompts, tools, hooks, and connectors only where officially supported and safe.
...
**Official documentation baseline to verify:** Mistral AI official mistral-vibe repository README; Mistral Vibe changelog; Official repository documentation for Skills and custom agents; Official issue tracker only as secondary evidence for current limitations.

## 4. Known audit findings to correct

- The current package uses AGENTS.md and .vibe/skills but does not use Vibe's current custom-agent capabilities.
- Skills require strict valid YAML frontmatter; malformed metadata may be skipped, and recent versions have changed discovery behavior.
- The package is generic and lacks a version/product-surface statement.
- Hooks and connectors are executable or external and must not be added.
## 5. Required platform-native corrections

1. Verify the current stable Mistral Vibe release and official repository documentation before editing paths or frontmatter.
1. Retain AGENTS.md as concise project guidance only if Vibe officially discovers it in the intended project context.
1. Validate .vibe/skills/*/SKILL.md against the current Vibe Skill schema. Quote YAML descriptions containing colon-space or other ambiguous YAML characters. Retain only distinct instruction-only Skills.
1. Add project-scoped custom agents under the exact current official path only if the stable documentation confirms repository discovery. Do not infer project paths from VIBE_HOME user directories.
1. When custom agents are supported, define a coordinator and bounded specialists using valid TOML or Markdown formats, least-privilege tools, and read-only reviewers. Otherwise keep an honest non-agent package.
1. Do not add custom tools, hooks, connectors, MCP, prompts that execute commands, telemetry settings, or provider authentication.
1. Rewrite workflow Skills to reflect Vibe's native skill invocation rather than pretend to be deterministic commands.
1. Document known stable-version assumptions and any unresolved discovery limitations in README.md.
1. Ensure no Skill references nonexistent agents or tools.
...
- Project custom agents only if current stable docs confirm the path
### Omit or delete
...
- Every Skill frontmatter block is valid YAML and uses current fields.
- The README names the target stable Vibe release or documentation date.
- Agent files are present only if official repository discovery is verified.
- No workflow claims command execution or runtime verification.
