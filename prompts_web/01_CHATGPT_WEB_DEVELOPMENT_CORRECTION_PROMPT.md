...
`TARGET_DIRECTORY = chatgpt/web-development/`

**Target product surface:** ChatGPT Projects, Custom GPTs, ChatGPT Skills, Workspace Agents, and apps/connectors only where currently supported through the ChatGPT UI and the user's plan or workspace administration.
...
**Official documentation baseline to verify:** OpenAI Help Center: Projects in ChatGPT; OpenAI Help Center: Skills in ChatGPT; OpenAI Help Center: Apps in ChatGPT; Current ChatGPT Enterprise/Business release notes where Workspace Agents are relevant.

## 4. Known audit findings to correct

- The current package is manual and plan-dependent; it cannot provide automatic repository orchestration.
- The package contains project instructions, knowledge files, Skills, Custom GPT setup, and Workspace Agent setup, but their supported surfaces and plan limitations are not sufficiently separated.
- Large portions are generic copies rather than instructions optimized for manual ChatGPT import and use.
- Connectors and apps must remain optional, unconnected, and disabled by default.
## 5. Required platform-native corrections

1. Treat every retained file as a manual configuration or import package. State clearly that ChatGPT does not auto-discover repository files from this folder.
1. Audit project-instructions.md for use as ChatGPT Project instructions. Make it concise, self-contained, and suitable for manual pasting without references to unavailable repository tools.
1. Audit knowledge/*.md as uploadable Project or GPT knowledge. Remove behavioral instructions that belong in project instructions and keep knowledge files factual, non-conflicting, and non-duplicated.
1. Validate every skills/*/SKILL.md against current ChatGPT Skills requirements. Retain only skills that provide distinct on-demand value and use valid metadata. Do not include scripts or executable resources.
1. Rework custom-gpt-setup.md and workspace-agent-setup.md so each targets a genuinely distinct current product surface. Delete either document if the surface is unavailable, redundant, deprecated, or unsupported for repository distribution.
1. Document plan, workspace-role, administrator, region, and rollout limitations without claiming universal availability.
1. Do not create fake agent manifests, hooks, permissions files, MCP configuration, or action schemas. Apps/connectors may be described only as optional manual integrations and must not be enabled or trusted.
1. Update README.md with a file-to-UI mapping, manual setup sequence, supported surfaces, limitations, and a static verification checklist.
...
- Custom GPT configuration guidance
- ChatGPT Skills where available
- Workspace Agent setup guidance where available
### Omit or delete

- Repository auto-discovery
- Repository subagent files
- Executable hooks
- Preconfigured MCP or connector authentication
...
- Every retained artifact must name the exact ChatGPT surface where it is used.
- No file may imply automatic loading from the repository.
- Custom GPT, Project, Skill, and Workspace Agent instructions must not conflict or duplicate the same full instruction body.
- Manual setup must be possible without secrets or external connections.
