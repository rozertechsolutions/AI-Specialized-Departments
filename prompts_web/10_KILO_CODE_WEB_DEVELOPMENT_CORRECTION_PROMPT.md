...
`TARGET_DIRECTORY = kilo-code/web-development/`

**Target product surface:** Kilo Code custom rules, AGENTS.md, custom subagents, commands/workflows, Agent Skills, permissions, and configuration through current official formats.
...
**Official documentation baseline to verify:** Kilo Code Docs: Custom Subagents; Kilo Code Docs: Custom Rules; Kilo Code Docs: Skills; Kilo Code Docs: Agent permissions and configuration.

## 4. Known audit findings to correct

- The lead is configured as a subagent and commands launch it as a subtask, preventing it from coordinating specialist subagents under the current depth behavior.
- The configuration uses the outdated schema URL https://kilo.ai/config.json instead of the current official application schema URL.
- AGENTS.md and .kilo/rules/web-development.md are byte-identical and can load redundant instructions.
- The configuration contains an empty MCP object.
- Role contracts and command procedures need stronger evidence and stop behavior.
## 5. Required platform-native corrections

1. Update kilo.jsonc to the exact current official schema URL and supported configuration keys.
1. Remove the empty MCP object. Do not create MCP configuration for this package.
1. Configure web-development-lead as a primary agent, not a subagent. Configure specialist roles as subagents that return results to the lead.
1. Update commands so coordinator workflows target the primary lead with subtask disabled. Specialist review commands may target an appropriate subagent only when the result returns to the calling context.
1. Use the current permission system, not deprecated tool booleans. Deny shell and external actions. Grant write access only to implementation specialists and read-only access to reviewers.
1. Eliminate duplicate instruction loading. Use either AGENTS.md for concise project-wide guidance and .kilo/rules for narrow platform rules, or remove one source. They must not contain identical full bodies.
1. Validate all .kilo/agents, .kilo/commands, .kilo/rules, and .kilo/skills paths and frontmatter against current Kilo Code documentation.
1. Define bounded subagent delegation, anti-recursion, evidence return, reviewer independence, and final reconciliation.
1. Keep Skills instruction-only and remove overlapping workflow content where commands already own the interaction.
...
- AGENTS.md when non-duplicative
### Omit or delete

- Subagent lead
- Empty MCP
- Deprecated tool configuration
- Duplicate AGENTS and rule bodies
- Shell and external actions
...
- The lead is selectable as a primary agent and can invoke the exact specialists.
- Commands do not accidentally start the lead as a subtask.
- Schema URL and every key match current official documentation.
- No instruction body is loaded twice through AGENTS.md and rules.
