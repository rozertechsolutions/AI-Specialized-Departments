...
`TARGET_DIRECTORY = github-copilot/web-development/`

**Target product surface:** GitHub Copilot repository instructions, path-specific instructions, custom agents, prompt files, and Agent Skills across GitHub.com cloud agent, Copilot CLI, VS Code, Visual Studio, and JetBrains surfaces where supported.
...
**Official documentation baseline to verify:** GitHub Docs: Repository custom instructions; GitHub Docs: Custom agents for Copilot cloud agent and CLI; GitHub Docs: Prompt files; GitHub Docs: Agent Skills; GitHub Copilot customization cheat sheet.

## 4. Known audit findings to correct

- The current package mixes multiple Copilot product surfaces without a precise compatibility matrix.
- Custom agents, prompt files, instructions, and Skills are not supported identically across GitHub.com, CLI, and IDEs.
- The lead may be independently selectable but must not claim automatic nested delegation unless the selected product surface explicitly supports it.
- Repository-wide and path-specific instructions may duplicate content and create conflicts.
## 5. Required platform-native corrections

1. Create an explicit support matrix in README.md for every retained component: GitHub.com Copilot coding agent, Copilot CLI, VS Code, Visual Studio, and JetBrains IDEs.
1. Validate .github/copilot-instructions.md as repository-wide guidance and .github/instructions/*.instructions.md as genuinely path-specific guidance. Remove repeated full instruction bodies.
1. Validate .github/agents/*.agent.md frontmatter and supported tools for each intended surface. If nested custom-agent delegation is not officially supported, define the lead as a manually selected coordinator and specialists as independently invocable agents.
1. Validate .github/prompts/*.prompt.md only for the IDE surfaces that support prompt files. Each prompt must state its expected input, selected agent if applicable, output, and evidence requirements.
1. Validate .github/skills/*/SKILL.md for the surfaces that support Agent Skills. Do not include scripts or pre-approved executable tools.
1. Keep implementation agents least-privileged and reviewers read-only where the custom-agent schema supports tool restrictions. Do not invent permission fields for surfaces that ignore them.
1. Do not create workflows, Actions, MCP configuration, GitHub app configuration, tokens, or automatic pull-request behavior.
1. Document manual selection and invocation instructions for each product surface. Make unsupported combinations explicit.
1. Ensure final review agents cannot modify the change they review and that completion requires evidence, not inferred success.
...
- Assumed universal surface support
- Automatic agent-to-agent delegation without documentation
- GitHub Actions
- MCP
- Tokens
- Automatic PR or deployment actions
...
- Every file is mapped to at least one officially supported Copilot surface.
- Instructions do not conflict when combined.
- Prompt files and agents use current frontmatter schemas.
- The README contains exact manual invocation and limitation notes.
- Unsupported IDE or cloud combinations are not presented as working.
