...
16. Runtime, build, test, browser, integration, skill, MCP, power, hook, and subagent behavior that was not actually performed is marked **NOT EXECUTED**, not **PASS**.
...
No MCP server, connector, remote tool, browser automation, shell execution, plugin, provider, model endpoint, power, hook, or external integration is enabled by this package. Users must review and configure integrations themselves.
