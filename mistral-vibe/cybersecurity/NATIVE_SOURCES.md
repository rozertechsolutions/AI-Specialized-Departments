...
- Isolated package inspection confirmed config schema fields and prompt search behavior without running agents or calling models. Current changelog was reviewed through `mistral-vibe==2.22.0`; no model call or authenticated session was started.
...
- Project instructions: current Vibe CLI documentation loads up to two `AGENTS.md` files, including the first trusted project-level file found by walking upward from the current working directory. Area `AGENTS.md` files are retained as durable project baselines for default or non-coordinator use.
- Prompt separation: coordinator prompts remain required `system_prompt_id` targets and retain coordinator-specific mission, role ownership, workflow coverage, structured outputs, delegation, and completion details. Matching area `AGENTS.md` files must not be byte-identical full prompt copies.
