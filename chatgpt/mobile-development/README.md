...
- Personal Skills: native for eligible ChatGPT Business, Enterprise, Healthcare, and Edu users. The Help Center confirms creation, editor, and upload paths, but the consulted OpenAI documentation does not provide enough package-format detail to validate exportable Skill packages in this repository.
...
4. Skills: do not upload a repository Skill package from this directory. If Skills are available in the target account, create the ten reusable workflows manually in ChatGPT's native Skill editor or with `skill-creator`, using the workflow definitions in `knowledge/QUALITY_AND_COMPLETION_STANDARDS.md`.
...
| Skills | native, conditional | Defined as manual Skill-editor source only; no package is emitted. |
...
- No `skills/` directory is included because no uploadable ChatGPT Skill package format was validated from the consulted official OpenAI documentation.
