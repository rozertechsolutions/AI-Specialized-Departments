...
| `skills/*/SKILL.md` | ChatGPT Skills | Upload or create each Skill where Skills are available for the user, plan, and workspace. |
...
| ChatGPT Skills | Conditionally native; beta availability depends on plan, workspace role, admin controls, region, and rollout. |
| Workspace Agents | Conditionally native for eligible Business or Enterprise workspaces with admin enablement. |
| Apps/connectors | Manual-only optional integrations; none are configured, connected, or trusted by default. |
| Actions | Manual-only optional Custom GPT integrations; none are configured. |
| Instructions | Manual-only in this package; ChatGPT receives them only after the user pastes them into the relevant UI. |
| Agents/subagents | Workspace Agents are conditionally native through the ChatGPT Agents UI; repository subagent files are unsupported and omitted. |
| Commands/workflows | Manual-only through Project, GPT, Agent, or Skill instructions; no command files are included. |
| Hooks | Unsupported for ChatGPT repository packages and omitted. |
| MCP | Unsupported as preconfigured repository authentication for this package and omitted. |
| Permissions | Manual workspace or GPT UI configuration only; no permission manifests are included. |
| Repository auto-discovery | Unsupported. |
| Automatic deployment or publication | Unsupported and omitted. |

## Manual Setup Sequence

1. Choose one primary surface: Project, Custom GPT, or Workspace Agent.
...
5. Install or create Skills only if the ChatGPT Skills UI is available and the workspace allows it.
6. Leave Apps, Actions, connectors, external APIs, sync, write actions, deployment, and publication disabled until a human reviews data access, scopes, retention, and approval requirements.
...
- Apps may require user connection, OAuth, workspace enablement, and explicit confirmation before external actions. Defaults can vary by plan and workspace policy.
...
- PASS: Every retained package file maps to a current ChatGPT UI surface.
- PASS: No retained package file implies repository auto-loading from this directory.
...
- PASS: Skills use `SKILL.md` with required `name` and `description` frontmatter.
