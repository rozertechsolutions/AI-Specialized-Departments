...
- Creating or editing GPTs requires a ChatGPT account with GPT creation access and, in managed workspaces, the required workspace permission.
- GPT building and editing are web-only. Mobile clients can use GPTs but may not support building or editing.
- GPT behavior depends on the exact capabilities, knowledge, Apps, and Actions enabled in the GPT builder.
...
| Instructions | Paste `project-instructions.md`, then add any organization-specific rules that are safe to include. |
...
4. Leave Actions, Apps, connectors, sync, and write actions off unless a human has reviewed data access, scopes, retention, confirmation prompts, and workspace restrictions.
